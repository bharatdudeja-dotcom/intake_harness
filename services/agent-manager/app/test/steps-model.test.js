...
/**
 * Increment 11 (D45) tests: the ordered Step/Recipe model, per-step approval + the
...
describe('ordering (D45): start_recipe + append_step', () => {
    test('a recipe starts empty and experimental; steps append in order 0,1,2', async () => {
        const recipe = parseResult(await callTool('start_recipe', { project: 'Ordering', title: 'ordered-demo' }))
        expect(recipe.status).toBe('experimental')

        const appended = []
        for (let i = 0; i < 3; i++) {
            appended.push(parseResult(await callTool('append_step', {
                recipe_id: recipe.id, kind: 'message', content: `step ${i}`, source: 'other'
            })))
        }
    ...
    test('a fresh experimental step expires roughly 30 days out', async () => {
        const recipe = parseResult(await callTool('start_recipe', { project: 'Expiry', title: 'expiry-demo' }))
    ...
    test('append_step requires content or asset', async () => {
        const recipe = parseResult(await callTool('start_recipe', { project: 'Validation', title: 'no-content' }))
        const result = await callTool('append_step', { recipe_id: recipe.id, kind: 'message' })
        expect(result.result.isError).toBe(true)
    })
})

describe('per-step approval + composed cookbook view (D45)', () => {
    test('approve_steps on 0 and 2 -> get_recipe("approved") returns exactly those, in order', async () => {
        const recipe = parseResult(await callTool('start_recipe', { project: 'Compose', title: 'compose-demo' }))
        const s0 = parseResult(await callTool('append_step', { recipe_id: recipe.id, kind: 'message', content: 'A', source: 'other' }))
        await callTool('append_step', { recipe_id: recipe.id, kind: 'message', content: 'B', source: 'other' })
        const s2 = parseResult(await callTool('append_step', { recipe_id: recipe.id, kind: 'message', content: 'C', source: 'other' }))
    ...
    test('the recipe now appears in list_resources/resources/list as a composed how-to (approved steps only)', async () => {
        const recipe = parseResult(await callTool('start_recipe', { project: 'Compose', title: 'compose-demo-2' }))
        const s0 = parseResult(await callTool('append_step', { recipe_id: recipe.id, kind: 'message', content: 'A', source: 'other' }))
        await callTool('append_step', { recipe_id: recipe.id, kind: 'message', content: 'B (still a draft)', source: 'other' })
        ...
        expect(nativeList).toContain(`resource://company/recipe/${recipe.id}`)

        const read = await rpc('resources/read', { uri: `resource://company/recipe/${recipe.id}` })
    ...
    test('discard_step excludes a step from views; an approved step cannot be discarded', async () => {
        const recipe = parseResult(await callTool('start_recipe', { project: 'Discard', title: 'discard-demo' }))
        const keep = parseResult(await callTool('append_step', { recipe_id: recipe.id, kind: 'message', content: 'keep', source: 'other' }))
        const toss = parseResult(await callTool('append_step', { recipe_id: recipe.id, kind: 'message', content: 'toss', source: 'other' }))
...
describe('retention / expiry / purge (D44/D45)', () => {
    test('purge_expired removes an artificially-expired experimental step but keeps the approved one', async () => {
        const recipe = parseResult(await callTool('start_recipe', { project: 'Retention', title: 'purge-demo' }))
        const keep = parseResult(await callTool('append_step', { recipe_id: recipe.id, kind: 'message', content: 'keep me', source: 'other' }))
        parseResult(await callTool('append_step', { recipe_id: recipe.id, kind: 'message', content: 'expire me', source: 'other' }))
        await callTool('approve_step', { step_id: keep.id })
        ...
        expect(result.purged_recipes).toBe(0)
    ...
    test('purge_expired removes a recipe left with no approved/active steps at all', async () => {
        const recipe = parseResult(await callTool('start_recipe', { project: 'Retention', title: 'fully-expired' }))
        await callTool('append_step', { recipe_id: recipe.id, kind: 'message', content: 'nothing kept', source: 'other' })

        const farFuture = new Date(Date.now() + 40 * 24 * 60 * 60 * 1000).toISOString()
        const { purgeExpired } = require('../lib/retention')
        const result = await purgeExpired(farFuture)
        expect(result.purged_recipes).toBe(1)

        const gone = await callTool('get_resource', { id: recipe.id })
        expect(gone.result.isError).toBe(true)
    })

    test('purge_expired is idempotent - a second run finds nothing left to purge', async () => {
        const recipe = parseResult(await callTool('start_recipe', { project: 'Retention', title: 'idempotent-demo' }))
        await callTool('append_step', { recipe_id: recipe.id, kind: 'message', content: 'temp', source: 'other' })
        ...
        expect(second.purged_recipes).toBe(0)
    })
...
describe('faithful capture: binary asset round-trip (D45)', () => {
    test('an image step round-trips its binary asset', async () => {
        ...
        const lightweight = parseResult(await callTool('list_steps', { recipe_id: recipe.id }))
    ...
    test('a diagram step can keep its mermaid source alongside a rendered asset', async () => {
        const recipe = parseResult(await callTool('start_recipe', { project: 'Assets', title: 'diagram-demo' }))
        const base64 = Buffer.from('fake-png-bytes').toString('base64')
        const appended = parseResult(await callTool('append_step', {
            recipe_id: recipe.id, kind: 'diagram', content: 'graph TD; A-->B;', format: 'mermaid',
...
describe('save_resource back-compat wrapper (D45)', () => {
    test('save_resource creates a single-step recipe; certify syncs step 0', async () => {
        ...
        const approved = parseResult(await callTool('get_recipe', { id: saved.id, view: 'approved' }))
    ...
    test('list_recipes excludes handoff-prompts and reflects status', async () => {
        await callTool('save_resource', { type: 'decision', title: 'a recipe', content: 'x', project: 'ListRecipes' })
        await callTool('save_resource', { type: 'handoff-prompt', title: 'a task', content: 'do the thing', target_agent: 'other' })

        const recipes = parseResult(await callTool('list_recipes', { project: 'ListRecipes' }))
        expect(recipes.every(r => r.type !== 'handoff-prompt')).toBe(true)
        expect(recipes.some(r => r.title === 'a recipe')).toBe(true)
    })
})

describe('project records are the source of truth (D49/D52)', () => {
    test('start_recipe auto-creates a project record', async () => {
        await callTool('start_recipe', { project: 'Auto From Recipe', title: 'r' })
        const projects = parseResult(await callTool('list_projects', {}))
        expect(projects.map(p => p.name)).toContain('Auto From Recipe')
    })
...
describe('lib/steps - pure helpers', () => {
    ...
    test('recipeStatusFromSteps is approved once any single step is approved', () => {
        expect(stepsLib.recipeStatusFromSteps([{ status: 'experimental' }])).toBe('experimental')
        expect(stepsLib.recipeStatusFromSteps([{ status: 'experimental' }, { status: 'approved' }])).toBe('approved')
    })
    ...
    test('makeStepId/parseStepId round trip', () => {
        const id = stepsLib.makeStepId('recipe-123', 4)
        expect(stepsLib.parseStepId(id)).toEqual({ recipeId: 'recipe-123', order: 4 })
